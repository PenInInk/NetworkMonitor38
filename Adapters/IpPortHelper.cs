...
namespace NetworkAdapters
{
    public class IpPortHelper
    {
        ...
        public static bool IsUdpPortInUse(int portNr)
        {
            TcpHelperUtil tcpHelper = new TcpHelperUtil();
            var details = tcpHelper.GetPortDetails(portNr);
            if (details.Item1)
            {
                log.Info(string.Format("Port {0} in Use", portNr));
                log.Info(details.Item2.ToString());
                return false;
            }
            else
            {
                //log.Info(string.Format("Port {0} is free ", portNr));
                return true;
            }
        }
        ...
        private class TcpHelperUtil
        {
            ...
            public Tuple<bool, PortDetail> GetPortDetails(int port)
            {
                ...
                string commandArgument = string.Format("/c netstat -an -o -p udp|findstr \":{0}",port);
